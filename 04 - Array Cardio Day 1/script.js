...
const bornIn1500s = inventors.filter(
  (obj) => Math.floor(obj.year / 100) === 15
);
...
const orderLifeLength = inventors.sort((a, b) =>
  a.passed - a.year >= b.passed - b.year ? 1 : -1
);
console.table(orderLifeLength);

// 6. create a list of Boulevards in Paris that contain 'de' anywhere in the name
// https://en.wikipedia.org/wiki/Category:Boulevards_in_Paris

// run in the browser console to work
...
const transportation = data.reduce((obj, vehicle) => {
  vehicle in obj ? ++obj[vehicle] : (obj[vehicle] = 1);
  return obj;
}, {});
